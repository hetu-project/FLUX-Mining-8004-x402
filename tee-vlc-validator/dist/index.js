...
const wallet = ethers_1.ethers.Wallet.fromPhrase(MNEMONIC).connect(provider);
...
async function validateAgentVLCProtocol(agentEndpoint, agentId) {
    ...
    try {
        ...
        console.log(`📊 Getting initial VLC state...`);
        ...
        const initialClock = initialState.clock;
        ...
        console.log(`📤 [Step 1] Sending ambiguous task: "${ambiguousTask}"`);
        ...
        const response1 = await step1Res.json();
        // STEP 3: Verify first response requests more information
        console.log(`📥 Agent Response: ${response1.outputType}`);
        ...
        console.log(`✅ Correct: Agent requested additional information`);
        ...
        console.log(`📊 VLC After Step 1 (need_more_info):`, afterFirstStep);
        ...
        console.log(`✅ Correct: VLC incremented properly [node ${nodeId}: ${initialValue} → ${afterFirstValue}]`);
        console.log();
        // STEP 5: Provide additional information
        const additionalInfo = "Route from point A(0,0) to point B(10,10), avoid obstacles at (5,5)";
        console.log(`📤 [Step 2] Providing additional info: "${additionalInfo}"`);
        ...
        const response2 = await step2Res.json();
        // STEP 6: Verify final response is ready
        console.log(`📥 Agent Response: ${response2.outputType}`);
        ...
        console.log(`✅ Correct: Agent provided final output`);
        ...
        console.log(`📊 VLC After Step 2:`, afterSecondStep);
        ...
        console.log(`✅ Correct: VLC incremented properly [node ${nodeId}: ${afterFirstValue} → ${afterSecondValue}]`);
        console.log();
        ...
        console.log(`✅ Causal consistency maintained throughout test`);
        console.log();
        // ALL TESTS PASSED
        console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
        console.log(`✅ VLC VALIDATION PASSED`);
        console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
        console.log(`Agent ID: ${agentId}`);
 ...
 */
function verifyCausalConsistency(initial, step1, step2, agentId) {
...
app.post('/validate-agent', async (req, res) => {
    try {
        ...
        const result = await validateAgentVLCProtocol(agentEndpoint, agentId.toString());
        console.log(`\n✅ Validation Complete`);
        console.log(`   Request Hash: ${requestHash}`);
        // Generate response hash for signature
        const responseData = `vlc-response-${agentId}-${wallet.address}-${result.score}`;
        const responseHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(responseData));
        // VLC_TAG: RIGHT-padded (text at beginning, zeros at end) to match contract
        const vlcTagBytes = ethers_1.ethers.toUtf8Bytes("VLC_PROTOCOL");
        const vlcTag = ethers_1.ethers.hexlify(ethers_1.ethers.concat([vlcTagBytes, new Uint8Array(32 - vlcTagBytes.length)]));
        // Sign the validation result with TEE wallet
        const messageHash = ethers_1.ethers.solidityPackedKeccak256(['bytes32', 'uint8', 'bytes32', 'bytes32'], [requestHash, result.score, responseHash, vlcTag]);
        const signature = await wallet.signMessage(ethers_1.ethers.getBytes(messageHash));
        console.log(`   🔐 TEE Signature: ${signature.substring(0, 20)}...`);
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n${'='.repeat(60)}`);
    console.log(`🚀 TEE VLC Validator Running`);
